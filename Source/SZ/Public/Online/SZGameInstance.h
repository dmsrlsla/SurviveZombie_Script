...
enum class EGameState : uint8
{
	...
	EGameMenu					UMETA(DisplayName = "Game Menu"),
...
UCLASS(config=Game)
class SZ_API USZGameInstance : public UGameInstance
{
	GENERATED_BODY()
	
public:
	USZGameInstance(const FObjectInitializer& ObjectInitializer);


	//Init Function
	virtual void Init() override;
	/*위젯 레퍼런스, 이는 스크린에 디스플레이 될것이다.*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SZ Game Instance")
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "SZ Game Instance")
		TSubclassOf<class UUserWidget> cGameMenu;


	/* 상태 변화 함수 */
	UFUNCTION(BlueprintCallable, Category = "SZ Game Instance")
...
private:
	...
	//class UEnterStateUserWidget CurrentWidget;
	//현재의 게임 스테이트
